...
function createExpressSubApp(rootApp, registry) {
    ...
    var sessionMiddleware = rootApp.get('sessionMiddleware');
    if (!sessionMiddleware) {
        sessionMiddleware = require('cookie-session')({keys: [registry.title + '1234!@#$contact'], maxAge: 14 * 24 * 60 * 60});
        app.use(sessionMiddleware);
    }
    registerSockets(registry, sessionMiddleware);
